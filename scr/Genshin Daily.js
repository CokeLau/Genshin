...
const imgPath = iCloud.joinPath(iCloud.documentsDirectory(), "Genshin Daily")
if (!iCloud.fileExists(imgPath) || !iCloud.isDirectory(imgPath)) { iCloud.createDirectory(imgPath) }

const CookiePath = iCloud.joinPath(iCloud.documentsDirectory(), "Cookie.txt")
...
//背景设置
  if(action == 2){
  iCloud.writeImage(iCloud.joinPath(imgPath, "Genshin.jpg"), await Photos.fromLibrary())
}
...
w.backgroundImage = iCloud.readImage(iCloud.joinPath(imgPath,"Genshin.jpg"))
...
console.log(sign)
  ...
  var resinIcon = stack.addImage(await loadResinIcon())
resinIcon.imageSize = new Size(22,22)
...
let uidstack = infostack.addStack()

uidstack.cornerRadius = 8
let uid = uidstack.addText(`  UID:${info[0]}  `)
uid.font = Font.boldRoundedSystemFont(8)

infostack.addSpacer()

let serverstack = infostack.addStack()
serverstack.cornerRadius = 8
if(info[1] === "cn_qd01"){
  server = "  世界树  "
}else{
  server = "  天空岛  "
}
let servers = serverstack.addText(`${server}`)
servers.font = Font.boldRoundedSystemFont(8)

infostack.addSpacer()

var userStack = infostack.addStack()
userStack.cornerRadius = 8
if(!sign.is_sign){
var MHY = userStack.addText(`  未签到  `)
MHY.textColor = Color.orange()}else{
  var MHY = userStack.addText(`  已签到  `)}
  MHY.font = Font.mediumRoundedSystemFont(8)
...
var resinIcon = resinStack.addImage(await loadResinIcon())
resinIcon.imageSize = new Size(22,22)
...
var homeIcon = homeStack.addImage(await loadCoinIcon())
homeIcon.imageSize = new Size(22,22)
...
var taskIcon = stack.addImage(await loadTaskIcon())
taskIcon.imageSize = new Size(22,22)
...
var disIcon = stack.addImage(await loadDiscountIcon())
disIcon.imageSize = new Size(22,22)
...
var recIcon = stack.addImage(await loadTransformerIcon())
recIcon.imageSize = new Size(22,22)
